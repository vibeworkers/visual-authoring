...
5. 선택한 구현 레이어와 표현 규칙의 역할 분리
- Evidence: 산출물 또는 검증 로그에 사용한 경우 `image base`, `SVG/native 의미 레이어`, `selected expression system 또는 none-with-reason`의 역할 구분. Carbon은 선택했을 때만 증거 대상이다.
