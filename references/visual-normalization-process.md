...
- 이론 앵커 1: 프로젝트가 선택한 token/style contract(또는 `none-with-reason`)로 스타일 의도를 고정한다. Carbon은 가능한 선택지 중 하나다.
- 이론 앵커 2: Evidence-first 시각 루브릭으로 미감 논쟁 대신 검증 가능한 판정을 사용한다.
- 확장 앵커: 생성 이미지는 필요한 경우 물성/사람/공간/빛/흔적을 만들고, SVG/native 의미 레이어는 관계/경로/주의/라벨을 설명한다.
- 모델 앵커: 이미지 생성은 필수 경로가 아니다. 사용할 때는 현재 런타임에서 실제로 선택한 모델·소유자·prompt·output/integration path를 ledger에 기록한다. 의미 경로/추상 라벨/조건 게이트는 편집 가능한 SVG/HTML/PowerPoint native layer에서 제어한다.
...
- 실패 대응: 신규 값은 선택한 namespace(`--cds-*`, `--viv-*`, `--brand-*`, `--custom-*` 등) 또는 기록된 PPTX theme/style map만 허용하고, 웹 표면은 모바일 캡처를 Must 검증에 포함.
