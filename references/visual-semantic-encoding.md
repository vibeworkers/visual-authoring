...
already weak. It is used before image generation, SVG/native overlay, an optional
expression-system integration, or final visual judgement.

## Purpose

The viewer should read the main meaning from visible cues first. Text labels,
arrows, numbers, and callouts may guide attention, but they must not be
the only reason a viewer understands the scene.

## Required Order

1. Fix the core message in one sentence.
2. Lock the artifact-level design-system grammar without locking a repeated layout.
3. Convert the message into scene-centered visual semantic units and name the
   reader inference or action for each unit.
4. Explore an open set of materiality candidates, including deterministic SVG,
   native objects, generated image, or hybrid; select by semantic and medium fit.
5. Define the label-masked reading criterion for relationship, action, or state change.
6. Generate or draw the base scene from visible cues only.
7. Hide labels, titles, arrows, and numbered callouts; judge what remains.
8. Add the deterministic semantic layer only after the base scene passes the masked read.
9. Materialize the selected expression system, or record `none-with-reason`, and
   run contact sheet / PNG evidence checks. Carbon is one optional system.
...
For a practice-report journey such as `가져온 자료 → 150분 실습 → 완성 결과물 → 7일 재사용`, the masked read should expose a transformation from input through work to output and later reuse. Exact Korean labels, `150분`, and `7일` remain deterministic text; the viewer is not expected to guess those exact values from imagery.
...
If translation, naturalization, or concretization changes visible or accessible
copy, the previous text-fit, geometry, screenshot/PDF, native-open, and
accessibility proof is stale for the affected surface. Reflow, rerender, and
recheck the target medium before release; an overflow-only check is insufficient.
